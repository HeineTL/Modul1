...
function updateView() {
    ...
    content.innerHTML += /*HTML*/`
        <div class="pokemons system">
            <h1>En vill ${pokemonName} (Lvl. ${pokemonLevel}) har dukket opp!</h1>
            <h2></h2>
            <p>Hva vil du gjøre?</p>
            <button onclick="catchPokemon('${pokemonName}', ${pokemonLevel})">Fang den</button>
            <button onclick="updateView()">Løp vekk</button>
            <button onclick="showInventory()">Vis inventar</button>
        </div>
    `;
}
...
function catchPokemon(pokemonName, pokemonLevel) {
    inventory.push({ pokemon: pokemonName, level: pokemonLevel });
    updateView();
}
...
function getInventory() {
    ...
    for (let i = 0; i < inventory.length; i++) {
        inv += `<b>${inventory[i].pokemon}</b> (Lvl. ${inventory[i].level}). `;
    }
...
function showInventory() {
    inventoryContent.innerHTML = /*HTML*/`
            <div class="inventory system">
                <h1>Pokemon Inventar</h1>
                <p>Du har ${inventory.length} pokemons</p>
                <p>${getInventory()}</p>
                <button onclick="closeInventory()">Lukk vindu</button>
                <button onclick="resetInventory()">Kast alle pokemons</button>
            </div>`;
    openInventory = true;
}
